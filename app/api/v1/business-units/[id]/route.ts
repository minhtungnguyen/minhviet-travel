...
import { businessUnitUpdateSchema } from '@/modules/organization/schemas/organization.schema'
...
export const PATCH = withParamsRoute<{ id: string }>(async (req: NextRequest, { id }, requestId) => {
  ...
  if (!idResult.success) throw AppError.validation('Invalid business unit id')
  ...
  const businessUnit = await service.updateBusinessUnit(actor, idResult.data, parsed.data, requestId)
...
export const DELETE = withParamsRoute<{ id: string }>(async (_req, { id }, requestId) => {
  ...
  const actor = await resolveActor()
  const service = await getService()
  const businessUnit = await service.archiveBusinessUnit(actor, idResult.data, requestId)
  return ok(businessUnit, requestId)
})
