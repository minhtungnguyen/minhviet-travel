...
import { announcementCreateSchema } from '@/modules/cms/schemas/cms.schema'
...
export const GET = withRoute(async (req: NextRequest, requestId) => {
  ...
  const announcements = await service.listAnnouncements(parsed.data.websiteId)
  return ok(announcements, requestId)
})
...
export const POST = withRoute(async (req: NextRequest, requestId) => {
  ...
  const service = await getService()
  const announcement = await service.createAnnouncement(actor, parsed.data, requestId)
