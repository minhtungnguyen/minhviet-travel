...
import { navigationItemCreateSchema } from '@/modules/navigation/schemas/navigation.schema'
...
export const GET = withParamsRoute<{ id: string }>(async (_req, { id }, requestId) => {
  ...
  const items = await service.listItems(idResult.data)
...
export const POST = withParamsRoute<{ id: string }>(async (req: NextRequest, { id }, requestId) => {
  ...
  const service = await getService()
  const item = await service.createItem(actor, idResult.data, parsed.data, requestId)
