...
import { cmsPageUpdateSchema } from '@/modules/cms/schemas/cms.schema'
...
export const GET = withParamsRoute<{ id: string }>(async (_req, { id }, requestId) => {
  ...
  const page = await service.getPage(idResult.data)
...
export const PATCH = withParamsRoute<{ id: string }>(async (req: NextRequest, { id }, requestId) => {
  ...
  const service = await getService()
  const page = await service.updatePage(actor, idResult.data, parsed.data, requestId)
  return ok(page, requestId)
})
...
export const DELETE = withParamsRoute<{ id: string }>(async (_req, { id }, requestId) => {
  ...
  const actor = await resolveActor()
  const service = await getService()
  await service.deletePage(actor, idResult.data, requestId)
