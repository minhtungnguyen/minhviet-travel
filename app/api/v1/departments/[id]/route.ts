...
import { departmentUpdateSchema } from '@/modules/organization/schemas/organization.schema'
...
export const PATCH = withParamsRoute<{ id: string }>(async (req: NextRequest, { id }, requestId) => {
  ...
  if (!idResult.success) throw AppError.validation('Invalid department id')
  const body = await req.json()
  const parsed = departmentUpdateSchema.safeParse(body)
  if (!parsed.success) throw AppError.validation('Invalid department payload', { issues: parsed.error.issues })
  const actor = await resolveActor()
  const service = await getService()
  const department = await service.updateDepartment(actor, idResult.data, parsed.data, requestId)
  return ok(department, requestId)
})
...
export const DELETE = withParamsRoute<{ id: string }>(async (_req, { id }, requestId) => {
  ...
  const actor = await resolveActor()
  const service = await getService()
  const department = await service.archiveDepartment(actor, idResult.data, requestId)
  return ok(department, requestId)
})
