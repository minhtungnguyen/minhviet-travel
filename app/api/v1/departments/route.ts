...
import { departmentCreateSchema } from '@/modules/organization/schemas/organization.schema'
...
export const GET = withRoute(async (req: NextRequest, requestId) => {
  ...
  const result = await service.listDepartments(actor, parsed.data.organizationId)
...
export const POST = withRoute(async (req: NextRequest, requestId) => {
  ...
  const actor = await resolveActor()
  const service = await getService()
  const department = await service.createDepartment(actor, parsed.data, requestId)
