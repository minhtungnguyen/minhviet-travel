...
import { currencyUpdateSchema } from '@/modules/master-data/schemas/master-data.schema'
...
export const PATCH = withParamsRoute<{ code: string }>(async (req: NextRequest, { code }, requestId) => {
  ...
  const currency = await service.updateCurrency(actor, code, parsed.data, requestId)
