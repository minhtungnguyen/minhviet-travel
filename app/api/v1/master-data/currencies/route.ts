...
import { currencyCreateSchema } from '@/modules/master-data/schemas/master-data.schema'
...
export const GET = withRoute(async (_req: NextRequest, requestId) => {
  ...
  return ok(await service.listCurrencies(), requestId)
})

export const POST = withRoute(async (req: NextRequest, requestId) => {
  const body = await req.json()
  const parsed = currencyCreateSchema.safeParse(body)
  if (!parsed.success) throw AppError.validation('Invalid currency payload', { issues: parsed.error.issues })
  const actor = await resolveActor()
  const service = await getService()
  const currency = await service.createCurrency(actor, parsed.data, requestId)
  return ok(currency, requestId)
})
