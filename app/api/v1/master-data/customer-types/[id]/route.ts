...
import { customerTypeUpdateSchema } from '@/modules/master-data/schemas/master-data.schema'
...
export const PATCH = withParamsRoute<{ id: string }>(async (req: NextRequest, { id }, requestId) => {
  ...
  if (!idResult.success) throw AppError.validation('Invalid customer type id')
  const body = await req.json()
  const parsed = customerTypeUpdateSchema.safeParse(body)
  ...
  const customerType = await service.updateCustomerType(actor, idResult.data, parsed.data, requestId)
