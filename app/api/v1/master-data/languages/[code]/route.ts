...
import { languageUpdateSchema } from '@/modules/master-data/schemas/master-data.schema'
...
export const PATCH = withParamsRoute<{ code: string }>(async (req: NextRequest, { code }, requestId) => {
  ...
  const language = await service.updateLanguage(actor, code, parsed.data, requestId)
