...
import { provinceUpdateSchema } from '@/modules/master-data/schemas/master-data.schema'
...
export const PATCH = withParamsRoute<{ id: string }>(async (req: NextRequest, { id }, requestId) => {
  ...
  if (!idResult.success) throw AppError.validation('Invalid province id')
  const body = await req.json()
  const parsed = provinceUpdateSchema.safeParse(body)
  ...
  const province = await service.updateProvince(actor, idResult.data, parsed.data, requestId)
