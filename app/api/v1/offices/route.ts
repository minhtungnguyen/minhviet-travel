...
import { officeCreateSchema } from '@/modules/organization/schemas/organization.schema'
...
export const GET = withRoute(async (req: NextRequest, requestId) => {
  ...
  const result = await service.listOffices(actor, parsed.data.organizationId)
  return ok(result, requestId)
})

export const POST = withRoute(async (req: NextRequest, requestId) => {
  const body = await req.json()
  const parsed = officeCreateSchema.safeParse(body)
  if (!parsed.success) throw AppError.validation('Invalid office payload', { issues: parsed.error.issues })
  const actor = await resolveActor()
  const service = await getService()
  const office = await service.createOffice(actor, parsed.data, requestId)
