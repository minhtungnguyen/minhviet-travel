...
import { positionUpdateSchema } from '@/modules/organization/schemas/organization.schema'
...
export const PATCH = withParamsRoute<{ id: string }>(async (req: NextRequest, { id }, requestId) => {
  ...
  if (!idResult.success) throw AppError.validation('Invalid position id')
  const body = await req.json()
  const parsed = positionUpdateSchema.safeParse(body)
  if (!parsed.success) throw AppError.validation('Invalid position payload', { issues: parsed.error.issues })
  const actor = await resolveActor()
  const service = await getService()
  const position = await service.updatePosition(actor, idResult.data, parsed.data, requestId)
  return ok(position, requestId)
})
...
export const DELETE = withParamsRoute<{ id: string }>(async (_req, { id }, requestId) => {
  ...
  const actor = await resolveActor()
  const service = await getService()
  const position = await service.archivePosition(actor, idResult.data, requestId)
  return ok(position, requestId)
})
