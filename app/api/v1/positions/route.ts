...
import { positionCreateSchema } from '@/modules/organization/schemas/organization.schema'
...
const listQuerySchema = z.object({ departmentId: uuidSchema })
...
export const GET = withRoute(async (req: NextRequest, requestId) => {
  ...
  const result = await service.listPositions(actor, parsed.data.departmentId)
...
export const POST = withRoute(async (req: NextRequest, requestId) => {
  ...
  const actor = await resolveActor()
  const service = await getService()
  const position = await service.createPosition(actor, parsed.data, requestId)
