...
import { userAdminUpdateSchema } from '@/modules/access-control/schemas/access-control.schema'
...
export const PATCH = withParamsRoute<{ id: string }>(async (req: NextRequest, { id }, requestId) => {
  ...
  if (!parsed.success) throw AppError.validation('Invalid user payload', { issues: parsed.error.issues })
  const actor = await resolveActor()
  const service = await getService()
  const user = await service.updateUser(actor, idResult.data, parsed.data, requestId)
  return ok(user, requestId)
})
