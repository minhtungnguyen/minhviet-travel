...
import { grantWebsiteAccessBodySchema } from '@/modules/access-control/schemas/access-control.schema'
...
export const GET = withParamsRoute<{ id: string }>(async (_req, { id }, requestId) => {
  ...
  const access = await service.listWebsiteAccess(actor, idResult.data)
  return ok(access, requestId)
})
...
export const POST = withParamsRoute<{ id: string }>(async (req: NextRequest, { id }, requestId) => {
  ...
  const parsed = grantWebsiteAccessBodySchema.safeParse(body)
  if (!parsed.success) throw AppError.validation('Invalid website access payload', { issues: parsed.error.issues })
  const actor = await resolveActor()
  const service = await getService()
  const access = await service.grantWebsiteAccess(actor, { userProfileId: idResult.data, ...parsed.data }, requestId)
  return ok(access, requestId)
})
