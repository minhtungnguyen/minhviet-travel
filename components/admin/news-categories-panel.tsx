...
import { createCategoryAction, deleteCategoryAction, updateCategoryAction } from '@/app/admin/news/categories/actions'
...
function CategoryRow({ category, canWrite }: { category: NewsCategory; canWrite: boolean }) {
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      const result = await updateCategoryAction(category.id, { name, slug, description: description || undefined })
      if (!result.ok) {
  ...
  function handleDelete() {
    if (!window.confirm(`Xoá hẳn danh mục "${category.name}"? Không thể xoá nếu còn bài viết đang dùng.`)) return
    setError(null)
    startTransition(async () => {
...
export function NewsCategoriesPanel({
  websiteId,
  categories,
  canWrite,
}: {
  ...
  function handleCreate(e: React.FormEvent) {
    ...
    setError(null)
    startTransition(async () => {
      const result = await createCategoryAction({ websiteId, name, slug, sortOrder: categories.length })
      if (!result.ok) {
        setError(result.message)
        return
      }
